...
export function AcbfDomainsSection() {
  ...
  // ----- Selected domain -----
  const [selectedDomain, setSelectedDomain] = useState<AcbfDomain | null>(null);
  ...
  // ----- Form state -----
  const [form, setForm] = useState<DomainFormValues>({
    code: "",
    name: "",
    order: 0,
  });
  ...
  const fetchDomains = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/acbf-domains?${params.toString()}`);
      if (!res.ok) {
  ...
  }, [canRead, fetchDomains, refreshKey]);
  ...
  function resetForm() {
    setForm({ code: "", name: "", order: 0 });
    setFormErrors({});
  }

  function validateForm(): boolean {
    const result = domainFormSchema.safeParse(form);
    if (!result.success) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const res = await fetch("/api/acbf-domains", {
        method: "POST",
      ...
      toast.success("Domaine ACBF créé avec succès");
  ...
  async function handleView(domain: AcbfDomain) {
    setSelectedDomain(domain);
    setViewDialogOpen(true);

    // Fetch full detail with deliverables
    try {
      const res = await fetch(`/api/acbf-domains/${domain.id}`);
      if (res.ok) {
        const data = await res.json();
  ...
  function openEditDialog(domain: AcbfDomain) {
    setSelectedDomain(domain);
    setForm({
      code: domain.code,
      name: domain.name,
  ...
  async function handleEdit() {
    ...
    try {
      ...
      const res = await fetch(`/api/acbf-domains/${selectedDomain.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      ...
      toast.success("Domaine ACBF modifié avec succès");
    ...
    try {
      ...
      toast.success(
        archiveAction === "archive"
          ? "Domaine ACBF archivé avec succès"
          : "Domaine ACBF restauré avec succès"
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Gérer le référentiel des domaines ACBF
          </p>
        </div>
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 text-center max-w-md">
              Vous n&apos;avez pas la permission de consulter les domaines ACBF.
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base">
                  Liste des domaines ACBF
                </CardTitle>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {total} domaine{total !== 1 ? "s" : ""} au total
                </p>
              </div>
            </div>
            <Button
            ...
            <div className="flex flex-col items-center justify-center py-12 px-4">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 text-center max-w-sm">
                {search || statusFilter !== "active"
                  ...
                  : "Aucun domaine ACBF n'a été créé pour le moment."}
              </p>
              {(search || statusFilter !== "active") && (
                <Button
                  variant="outline"
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50/80 dark:bg-slate-800/50">
                      <TableHead className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 w-16">
                        Ordre
                      </TableHead>
                      ...
                      >
                        ...
                        <TableCell className="hidden md:table-cell">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-sm text-slate-600 dark:text-slate-300">
                              {domain._count?.deliverables ?? 0} livrable
                              {(domain._count?.deliverables ?? 0) !== 1 ? "s" : ""}
                            </span>
                          </div>
                        </TableCell>
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      className="h-8 w-8 text-slate-500 hover:text-emerald-600 dark:text-slate-400 dark:hover:text-emerald-400"
                                      onClick={() =>
                                        openArchiveDialog(domain, "restore")
                                      }
                                    >
                                      <RefreshCw className="h-4 w-4" />
                                    </Button>
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      onClick={() =>
                                        openArchiveDialog(domain, "archive")
                                      }
                                    >
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Nouveau domaine ACBF
            </DialogTitle>
            <DialogDescription>
              Créez un nouveau domaine ACBF dans le référentiel.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="create-name"
                placeholder="Nom du domaine ACBF"
                value={form.name}
                onChange={(e) =>
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Modifier le domaine ACBF
            </DialogTitle>
            <DialogDescription>
              Modifiez les informations du domaine ACBF.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <BookOpen className="h-5 w-5 text-emerald-600" />
              Détails du domaine ACBF
            </DialogTitle>
            <DialogDescription>
              Informations détaillées du domaine ACBF.
            </DialogDescription>
          </DialogHeader>
          {selectedDomain && (
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-900 dark:text-white mt-1">
                        {formatDate(selectedDomain.createdAt)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-3">
                    Livrables associés ({selectedDomain.deliverables?.length ?? selectedDomain._count?.deliverables ?? 0})
                  </p>
                  {selectedDomain.deliverables && selectedDomain.deliverables.length > 0 ? (
                    <div className="space-y-2">
                      {selectedDomain.deliverables.map((d) => (
                        <div
                          key={d.id}
                        ...
                        >
                          <div className="flex h-7 w-7 items-center justify-center rounded-md bg-emerald-100 dark:bg-emerald-900 shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              {d.priority && (
                                <Badge className="text-[9px] bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300 border-0">
                                  {d.priority}
                                </Badge>
                              )}
                              {d.status && (
      ...
      <AlertDialog open={archiveDialogOpen} onOpenChange={setArchiveDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {archiveAction === "archive"
                ? "Archiver le domaine ACBF"
                : "Restaurer le domaine ACBF"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {archiveAction === "archive"
